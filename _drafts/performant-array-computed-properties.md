...
You don't have to concern yourself when depending on `array`, `array.length`, or `array.[]`. But if you're using `array.@each.someProp`, you may need to take one of the following steps to avoid a performance overhead with large arrays.
...
In our case, we were able to simplify our dependencies, and our app became fast again. 
